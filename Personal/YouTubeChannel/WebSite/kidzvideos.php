...
echo "<h2>Rule #<del>7</del>9: Your kidz are NOT ALLOWED to watch my videos!<br/>";
echo "Rule #42: Kidz, you are REQUIRED to ask permission before NOT WATCHING MY VIDEOS!</h2>";
echo "<button id=\"playall\" onclick=\"playall()\">Play All (Videos for Kids are NOT ALLOWED on a Youtube Playlist)</button>";
echo "<table>";

echo "<tr><th>Name</th><th>Identifier</th><th>Additional</th><th>Dislikes</th><th>Likes</th></tr>";

$dbconn = pg_connect("{pgconnstr}");
$query = 'SELECT v.youtubeid,v.shortname,v.identifier,v.dislikes,v.likes FROM public.videolist_kidzvideos kv INNER JOIN public.videos v ON kv.videoid = v.id ORDER BY kv.ord';
...
while ($row = pg_fetch_assoc($result)) {
 ...
 echo $youtubeid;
 echo "&return=kidzvideos\">Description</a>";
 echo "</td>";
 echo "<td>";
 echo $row["dislikes"];
 echo "</td>";
